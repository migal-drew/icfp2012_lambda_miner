...
typedef bool (*TCheckFunction)(char** m, int width, int height, Point);
...
class MineMap
{
    ...
    private:
        ...
        int getOneChar();
		int m_Score;
		void MoveRobotAndUpdateMap(Point nextRobotPos);
		void SetBeard(char** newMap, Point point);
		void CutBeard(Point point);
